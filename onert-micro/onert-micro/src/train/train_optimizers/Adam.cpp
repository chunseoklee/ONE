 ...
 * v`(t) = v(t) / (1 - (beta_2) ^ t)
 *
 * w(t + 1) = w(t) - learning_rate * m`(t) / (sqrt(v` + epsilon))
 */
                             ...
                             core::OMRuntimeContext &context)
{
  ...
  for (auto &tensor_to_data : _tensor_index_to_gradient)
  {
    ...
    float learning_rate = training_config.learning_rate;
    ...
    for (uint32_t i = 0; i < flat_size; ++i)
    {
      ...
      f_weight_data[i] -=
        learning_rate * (exponent_corrected / (std::sqrt(exponent_square_corrected + epsilon)));
    }
  }
