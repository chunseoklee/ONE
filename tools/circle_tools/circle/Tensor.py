...
class Tensor(object):
    ...
    def Quantization(self):
        ...
        if o != 0:
            ...
            from circle.QuantizationParameters import QuantizationParameters
    ...
    def Sparsity(self):
        ...
        if o != 0:
            x = self._tab.Indirect(o + self._tab.Pos)
            from circle.SparsityParameters import SparsityParameters
    ...
    def VariantTensors(self, j):
        ...
        if o != 0:
            ...
            from circle.VariantSubType import VariantSubType
            obj = VariantSubType()
            obj.Init(self._tab.Bytes, x)
            return obj
        return None
...
def TensorAddCompressionType(builder, compressionType):
    builder.PrependInt8Slot(10, compressionType, 0)

def AddCompressionType(builder, compressionType):
    TensorAddCompressionType(builder, compressionType)
...
import circle.QuantizationParameters
import circle.SparsityParameters
import circle.VariantSubType
try:
...
class TensorT(object):
    ...
    def __init__(self):
        ...
        self.quantization = None  # type: Optional[circle.QuantizationParameters.QuantizationParametersT]
        self.isVariable = False  # type: bool
        self.sparsity = None  # type: Optional[circle.SparsityParameters.SparsityParametersT]
        self.shapeSignature = None  # type: List[int]
        self.hasRank = False  # type: bool
        self.variantTensors = None  # type: List[circle.VariantSubType.VariantSubTypeT]
    ...
    def _UnPack(self, tensor):
        ...
        if tensor.Quantization() is not None:
            self.quantization = circle.QuantizationParameters.QuantizationParametersT.InitFromObj(tensor.Quantization())
        self.isVariable = tensor.IsVariable()
        if tensor.Sparsity() is not None:
            self.sparsity = circle.SparsityParameters.SparsityParametersT.InitFromObj(tensor.Sparsity())
        ...
        if not tensor.VariantTensorsIsNone():
            ...
            for i in range(tensor.VariantTensorsLength()):
                if tensor.VariantTensors(i) is None:
                    ...
                    variantSubType_ = circle.VariantSubType.VariantSubTypeT.InitFromObj(tensor.VariantTensors(i))
    ...
    def Pack(self, builder):
        ...
        if self.variantTensors is not None:
            TensorAddVariantTensors(builder, variantTensors)
        TensorAddCompressionType(builder, self.compressionType)
