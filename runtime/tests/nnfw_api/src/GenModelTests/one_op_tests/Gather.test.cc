...
TEST_F(GenModelTest, OneOp_Gather_rank5_scalarindex)
{
  CircleGen cgen;
  ...
  int indice = cgen.addTensor({{}, circle::TensorType::TensorType_INT32, index_buf});
  int output = cgen.addTensor({{1, 1, 2, 2}, circle::TensorType::TensorType_FLOAT32});
...
TEST_F(GenModelTest, neg_OneOp_Gather_InvalidIndexType)
{
  CircleGen cgen;

  std::vector<float> index_data{1};

  auto index_buf = cgen.addBuffer(index_data);

  int input = cgen.addTensor({{3, 1, 1, 2, 2}, circle::TensorType::TensorType_FLOAT32});
  int indice = cgen.addTensor({{1}, circle::TensorType::TensorType_FLOAT32, index_buf});
